...
import { WorkspaceMetricCard } from "@/components/workspace/chrome";
              ...
              <div className="overflow-x-auto">
                <div className="min-w-[1120px]">
                  ...
                  {boardProfessionals.map((professional, index) => {
                    ...
                    const nextFreeDay = calendarDays.find((day) => {
                      if (day.date < today) return false;
                      return !(appointmentsByCell.get(`${professional.id}:${day.key}`)?.length);
                    });
                    const freeLabel = professional.id === UNASSIGNED_PROFESSIONAL_ID
                      ? "Setup pendiente"
                      : nextFreeDay
                        ? `Libre: ${nextFreeDay.label}`
                        : "Semana completa";
                    return (
                      <div
                        key={professional.id}
